...
[#img-rede-winserver]
image::winserver-2.png[align="center"]
+
Quando perguntado se deseja que o computador possa ser descoberto por outros dispositivos na rede, responda _Yes_.
+
.Descoberta de rede da máquina WinServer-G
[#img-discover-winserver]
image::winserver-3.png[align="center"]
+
...
+
A partir do menu Iniciar, invoque o Windows PowerShell. Em seguida, execute o comando `sconfig`:
+
....
PS C:\Users\Administrator> sconfig
....
+
O `sconfig` é o _Server Configuration Tool_, uma ferramenta de linha de comando para configuração de instalações do Windows Server Core. Escolha a opção `5`, para editar as configurações do Windows Update:
+
....
===============================================================================
...
1) Domain/Workgroup:                    Workgroup:  GRUPO
2) Computer Name:                       WINSERVER-A
(...)

Enter number to select an option: 5
....
+
Agora, marque as atualizações do sistema como `(M)anual` -- elas deverão ser expressamente baixadas e instaladas pelo administrador, quando desejado.
+
....
...
1. Usando o 1) _Control Panel_, ou 2) clicando com o botão direito em _Computer_ > _Properties_ no Windows Explorer ou 3) digitando `system` no menu iniciar, abra a tela de configuração do sistema como mostrado a seguir:
+
.Tela de configuração do sistema do WinServer
[#img-winserver-system]
image::winserver-11.png[align="center"]
+
<<<
...
image::winserver-12.png[align="center"]
<<<

3. Não reinicie o computador ainda. Na aba _Remote_, marque a caixa _Allow remote connections to this computer_ e desmarque _Allow connections only from computers running Remote Desktop with Network Level Authentication_, como na imagem abaixo. Depois, clique em _Apply_ e em seguida em _Restart Later_.
+
.Configurações de Remote Desktop do WinServer
[#img-winserver-rdesktop]
image::winserver-13.png[align="center"]
<<<

4. Agora, desabilite o firewall do Windows. Digite `firewall` no menu _Start_ (alternativamente, clique em _Windows Defender Firewall_ no _Control Panel_), em seguida em _Turn Windows Defender Firewall on or off_, e finalmente marque as caixas _Turn off Windows Defender Firewall_ para ambas as rede Privada e Pública, como se segue:
+
.Desabilitar o firewall do WinServer
[#img-winserver-firewall]
image::winserver-14.png[align="center"]

5. Clique em _OK_ e reinicie a máquina _WinServer-G_.
+
<<<

6. Após o _reboot_, acesse o _Server Manager_ (que deverá ser iniciado automaticamente após o logon), e no canto superior direito clique em _Manage_ > _Add Roles and Features_.
+
Na janela _Before you begin_, clique em _Next_ (opcionalmente, marque a caixa _Skip this page by default_).
+
Em _Select installation type_, mantenha _Role-based or feature-based installation_ e clique em _Next_.
+
.Instalação de roles e features no WinServer, parte 1
[#img-winserver-role1]
image::winserver-15.png[align="center"]
+
Em _Select destination server_, mantenha _Select a server from the server pool_ marcado e garanta que a máquina _WinServer-G_ está selecionada na caixa _Server Pool_. Clique em _Next_.
+
Em _Select server roles_, marque a caixa _Web Server (IIS)_. Quando surgir a pergunta _Add features that are required for Web Server (IIS)?_, clique em _Add Features_, e depois em _Next_.
+
.Instalação de roles e features no WinServer, parte 2
[#img-winserver-role2]
image::winserver-16.png[align="center"]
+
Em _Select features_, clique em _Next_.
+
Em _Web Server Role (IIS)_, clique em _Next_ para prosseguir.
+
A seguir, na janela _Select role services_, desça a barra de rolagem até o final e marque a caixa _FTP Server_, como se segue.
+
.Instalação de roles e features no WinServer, parte 3
[#img-winserver-role3]
image::winserver-17.png[align="center"]

7. Finalmente, clique em _Install_ e aguarde. Ao final do processo, clique em _Close_.

...
2. Agora, vamos iniciar uma captura de pacotes, aguardando o ataque. Ainda na máquina _FWGW1-G_, instale o `tcpdump` e monitore os pacotes vindos da DMZ, através da interface `enp0s8`.
ifdef::gabarito[]
+
....
...
....
+
Esse campo denota o número de bytes não confirmados como recebidos pelo receptor, efetivamente configurando a fila de envio do sistema. Para mais detalhes sobre esse campo em particular, instale o binário `netstat` (`apt-get install net-tools`) e verifique sua _manpage_ com o comando `man 8 netstat`.
+
...
8. Primeiro, vamos escalar privilégios dentro da máquina-alvo. Se você executar o comando `getuid`, irá notar que o `meterpreter` está executando como o usuário que abriu o PDF originalmente (provavelmente, o usuário `Administrator`).
+
....
meterpreter > getuid
...
============
 ...
 892   616   svchost.exe       x86   0        NT AUTHORITY\NETWORK SERVICE  C:\Windows\system32\svchost.exe
....
+
....

...
# iptables -A FORWARD -s 10.1.1.0/24 -d 172.16.1.10/32 -p udp -m multiport --dports 123 -j ACCEPT
....
endif::gabarito[]
...
# iptables -A FORWARD -i eth0 -d 172.16.1.20/32 -p tcp -m multiport --dports 21,443,80 -j ACCEPT
...
=== 5) Configuração do NAT no firewall *_FWGW1-G_*
...
+
....
# iptables -t nat -A PREROUTING -i eth0 -p tcp -m multiport --dports 21,443 -j DNAT --to-de stination 172.16.1.20
...
ifdef::gabarito[]
+
Abaixo, temos a configuração final sugeriada para o firewall:
+
[source,bash]
----
include::{srcdir}/s5/etc_iptables_rules.v4[]
----
endif::gabarito[]

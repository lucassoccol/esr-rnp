...
+
....
$ ssh -o PreferredAuthentications=keyboard-interactive,password -o PubkeyAuthentication=no l uke@10.0.42.2
...
4. Vamos testar o acesso de `leia` na máquina `fw`. Antes disso o primeiro passo, é claro, é criar o diretório `/config` e configurar sua montagem automática durante o _boot_ via `/etc/fstab`. Acesse `fw` como `root` e insira a linha a seguir no final do arquivo:
+
....
# hostname ; whoami
fw
